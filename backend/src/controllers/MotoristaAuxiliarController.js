...
//  console.log('ENTROU NO METODO');  
  ...
  const { nome, email, endereco, telefone1,
    ...
    ano, cor, bilingue, foto_blob, indicacao, situacaoId, perfilId, idioma1, idioma2, data_validade, 
    numero_carteira, motorista_principal_id} = req.body; 
//console.log('id -'+JSON.stringify(id, null, "    "));  
  ...
  await Motorista_auxiliar.update({
    ...
    motorista_principal_id: motorista_principal_id,
  },{
    where: { id: id}
  })
  .then( function (data){
...
controllers.create = async (req,res) => {  
  ...
  const { nome, email, endereco, telefone1, numero, 
    ...
    bairro, estadoId, cep, cpf, data_nascimento, carro, modelo, carroId, modeloId, placa, statusId,
    ano, cor, bilingue, indicacao, situacaoId, perfilId, foto_blob, idioma1, idioma2, data_validade, 
    numero_carteira, motorista_principal_id } = req.body;    


  //console.log("ROle es ==>"+role)
  //create
  await Motorista_auxiliar.create({
    nome: nome,              
            email: email,
            senha: senha,
            endereco: endereco,
