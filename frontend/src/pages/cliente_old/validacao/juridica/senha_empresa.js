  ...
  analyzeSenha(event) {
   ...
   //console.log('Senha '+event.target.value)
    ...
    //if (strongRegex.test(event.target.value)) { 
      ...
      if (umncaracterespecialRegex.test(event.target.value)) {
              validate.caracterespecialState= 'has-success'
              this.setState({ 
              // backgroundColor: "#aqua",             
                mensagem_caracterespecial: '',
                senhaumnespecial: true,                
              });        
              this.controle_caracter_especial();            
      }
...
controle_oito_caracteres() {   
  const { validate } = this.state;
    ...
    if (validate.oitocaracteresState == 'has-success') {
       return (
        <div className="alerta_senha_confirmado"><i className="fa fa-check-circle"  aria-hidden="true"></i> Míniomo 8 caracteres.</div>
        );  
    } else {
      return (
         <div className="alerta_senha"><i className="fa fa-check-circle"  aria-hidden="true"></i> Míniomo 8 caracteres.</div>
    ...
    return (
      ...
      if (validate.oitocaracteresState == 'has-success' && validate.umaletramaiusculaState == 'has-success' 
          ...
          if (this.state.campSenha.length > 0 && this.state.campSenhaTeste.length > 0 
            ...
            return (
              <Box bgcolor="error.main" color="error.contrastText" className="botao_cadastro_senha"  p={2} onClick={()=>this.sendUpdate()}>
              <div className="d-flex justify-content-center">
              <label> Próximo </label>
              </div>     
              </Box>           
   ...
   <div className="area_esquerda">     
          <div className="d-flex justify-content-around">
               <div className="botao_navegacao">
                 <Link to='/empresa_endereco'> <i className="fa fa-chevron-left fa-2x espacamento_seta"  aria-hidden="true"></i> </Link>
               </div>                  
               <div>
                 <div className="titulo_representante">                
                  <label>  {this.verifica_nome(this.state.campRazao_social)}, Cadastre a sua senha de acesso  </label>       
                 </div>
               </div>   
          ...
          <div>
                <Progress color="warning" value={80} className="progressbar"/>
          </div>
                    ...
                    endAdornment={
                      <InputAdornment position="end">
                        <IconButton
                          ...
                          edge="end"
                        >
                          {this.state.hiddenSenha ? <Visibility /> : <VisibilityOff />}
                        </IconButton>
                      </InputAdornment>
                    }
                    labelWidth={70}
                      ...
                      endAdornment={
                        <InputAdornment position="end">
                          <IconButton
                            ...
                            edge="end"
                          >
                            {this.state.hiddenSenhaConfirma ? <Visibility /> : <VisibilityOff />}
                          </IconButton>
                        </InputAdornment>
                      }
                      labelWidth={70}
