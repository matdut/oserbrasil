...
class listComponent extends React.Component  {
  ...
  constructor(props){
    ...
    this.state = {
      ...
      inicio: 1,
      color: 'light',
      listaMensagensMotorista:[],
      listaMensagensCliente:[],     
      validate: {
        descricaoState: ''
      }    
    }
  ...
  componentDidMount(){
    ...
    if (sessionStorage.getItem('logperfil') == 0) {
      ...
      this.props.history.push(`/login`);       
       ...
       this.loadMensagensMotoristas();  
       this.loadMensagensClientes();   
    }   
  }  
 
  loadMensagensMotoristas(){ 
   
    api.get('/mensagens_motorista/list')
    .then(res=>{
      if (res.data.success) {
        const data = res.data.data
        this.setState({listaMensagensMotorista:data})
      }
   ...
   loadMensagensClientes(){ 
   
    api.get('/mensagens_cliente/list')
    .then(res=>{
      if (res.data.success) {
        const data = res.data.data
        this.setState({listaMensagensCliente:data})
      }     
    })
    .catch(error=>{
      alert("Error server "+error)
  ...
  sendSave(){       
    ...
    const datapost = {
      descricao: this.state.campDescricao,
          ...
          api.post('/mensagens_motorista/create',datapost)
          .then(response=>{
            if (response.data.success) {                   
           
             this.loadMensagensMotoristas(); 
             this.setState({                
                mensagem_usuario: 'Mensagens Motorista incluída com sucesso!'
               });
  ...
  sendSaveCliente(){       
    ...
    const datapost = {
      descricao: this.state.campDescricao,
          ...
          api.post('/mensagens_cliente/create',datapost)
          .then(response=>{
            if (response.data.success) {                   
           
             this.loadMensagensClientes(); 
             this.setState({                
                mensagem_usuario: 'Mensagens Cliente incluída com sucesso!'
               });

            this.handleCloseModalInclusaoCliente();
             this.envia_mensagemClick();    
...
botao_modal_cliente(inicio) {    
    ...
    return (
      ...
      if (validate.descricaoState == 'has-success') { 
          return (
            ...
            <Box bgcolor="text.disabled" color="background.paper" className="botoes_habilitados_modal"  p={2} onClick={()=>this.sendSaveCliente()}>
                    <div className="d-flex justify-content-center">
                    <label> Incluir </label>
                    </div>     
            </Box>           
  ...
  //  const classes = useStyles();
      ...
      <div>   
          <Menu_administrador />  
            <div className="titulo_lista">
              ...
              <div className="titulo_bemvindo"> Mensagens </div>
              </div>      
            </div>
            <br/>
            ...
            <AppBar position="static" color="transparent">
              <TabList onChange={this.opcao_tabChange} aria-label="simple tabs example">           
                 <Tab label="Motorista" value="1" className="tabs_titulo_lista"/>          
                 <Tab label="Cliente" value="2" className="tabs_titulo_lista"/>               
              </TabList>
            </AppBar>
            ...
            <TabPanel value="1" className="tirar_espaco">        
               ...
               <MaterialTable          
                   ...
                   data={this.state.listaMensagensMotorista}     
                   localization={{
                     body: {
                   ...
                   options={{
                    ...
                    exportAllData: true,
                    exportFileName: 'Rel_motivos_ativos',
                    search: true,     
                 ...
                 />      
                  <div className="botao_lista_incluir">
                   <Fab style={{ textTransform: 'capitalize',  outline: 'none'}} className="tamanho_botao" size="large" color="secondary" variant="extended" onClick={()=>this.handleOpenModalInclusao()}>
                       <AddIcon/> <div className="botao_incluir"> Adicionar Mensagens Motorista </div>
                   </Fab>
                 </div>                        
        </TabPanel>  
        <TabPanel value="2" className="tirar_espaco">        
               ...
               <MaterialTable          
                   ...
                   data={this.state.listaMensagensCliente}     
                   localization={{
                     body: {
                       emptyDataSourceMessage: 'Nenhum registro para exibir',
                       addTooltip: 'Adicionar Valores Tarifários',
                   ...
                   actions={[                        
                    /* {
                     ...
                     }, */
                     {
                       icon: 'delete',                                                             
                       tooltip: 'Deleta Mensagem',          
                       onClick: (evt, data) => this.handleOpenModalDeleteCliente(data)                                     
                     }
                   ]} 
                   /*
                   editable={{                          
                 ...
                 />      
                  <div className="botao_lista_incluir">
                   <Fab style={{ textTransform: 'capitalize',  outline: 'none'}} className="tamanho_botao" size="large" color="secondary" variant="extended" onClick={()=>this.handleOpenModalInclusaoCliente()}>
                       <AddIcon/> <div className="botao_incluir"> Adicionar Menagens Cliente </div>
                   </Fab>
                 </div>                        
        </TabPanel>         
   ...
   ><div className="editar_titulo_inclusao"> Incluir Mensagens Motorista  
...
<ReactModal 
   isOpen={this.state.showModalInclusaoCliente}
   style={customStyles}
   contentLabel="Inline Styles Modal Example"                                  
   ><div className="editar_titulo_inclusao"> Incluir Mensagens Cliente  
       <IconButton aria-label="editar" onClick={()=>this.handleCloseModalInclusaoCliente()} className="botao_close_modal_tipo_veiculo">
         <CloseOutlinedIcon />
       </IconButton></div>       
       <div className="container_alterado">
          <div className="d-flex justify-content">        
            ...
            <div class="d-flex flex-column espacamento_caixa_texto">              
                 ...
                 <FormControl variant="outlined" className="posicao_caixa_texto">
                   ...
                   {this.botao_modal_cliente(this.state.inicio)}     
                 </div>
               </div>        
            </div>
</ReactModal>  
   ...
   ><div> 
       ...
       <div className="container_alterado">              
        ...
        <div className="moldura_modal_delecao">
          <div className="titulo_moldura_modal_delecao">Deseja mesmo excluir esta Mensagens? </div>
...
<ReactModal 
   isOpen={this.state.showMensagemDeletecliente}
   style={ConfirmacaodelStyles}
   contentLabel="Inline Styles Modal Example"                                  
   ><div> 
       <IconButton aria-label="editar" onClick={()=>this.handleCloseModalDeleteCliente()} className="botao_close_modal_deletar">
         <CloseOutlinedIcon />
       </IconButton></div>       
       <center><img src="/exclamation.png" /> </center>
       <div className="container_alterado">              
        ...
        <div className="moldura_modal_delecao">
          ...
          <div>Ao confirmar a exclusão o registro será apagado.  </div>
                         ...
                         <div className="retorno">{this.state.retorno}</div>
       <Box 
          className="botoes_delete_cancelar_modal" p={2} onClick={()=>this.handleCloseModalDeleteCliente()}>
         <div className="d-flex justify-content-center">
         <label> Cancelar </label>
         </div>     
       </Box>      
       <Box 
          className="botoes_delete_excluir_modal" p={2} onClick={()=>this.sendDeleteCliente(this.state.campDeletarId)}>
         <div className="d-flex justify-content-center">
         <label> Excluir </label>
         </div>     
       </Box>      
 ...
 // </div>
  ...
  handleOpenModalInclusaoCliente () { 
    this.setState({ 
      showModalInclusaoCliente: true,
      campDescricao: '',
      validacao_descricao: false,
    });  
  ...
  handleCloseModalInclusaoCliente () {
    this.setState({ 
      showModalInclusaoCliente: false
    });
  ...
  handleOpenModalDeleteCliente(data) { 
    this.setState({ 
      showMensagemDeletecliente: true,
      campDeletarId: data.id,
      retorno: '',
      campDescricao: '',
      validacao_descricao: false,
  ...
  handleCloseModalDeleteCliente() {
    this.setState({ 
      showMensagemDeletecliente: false
    });   
  }
  ...
  sendDelete(userId){  
    ...
    api.delete(`/mensagens_motorista/delete/${userId}`)
    .then(response =>{
      if (response.data.success) {
        ...
        this.setState({       
          mensagem_usuario: 'Mensagem Motorista excluída com sucesso!'
         });
    ...
    .catch ( error => {
      alert("Error mensagem/delete - "+error)
    })
  }

  sendDeleteCliente(userId){  

    console.log('id - '+userId);
    api.delete(`/mensagens_cliente/delete/${userId}`)
    .then(response =>{
      if (response.data.success) {
        ...
        this.setState({       
          mensagem_usuario: 'Mensagem Cliente excluída com sucesso!'
         });

        this.handleCloseModalDeleteCliente();
        this.envia_mensagemExclusaoClick();
